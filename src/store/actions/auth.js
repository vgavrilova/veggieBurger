...
export const authAction = (email, password, signup) => {
  return (dispatch) => {
    ...
    let url =
      "https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyCVZPhRTbBSLpnEia4mMTdIih9HVMMVkqw";
    if (!signup) {
      url =
        "https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyCVZPhRTbBSLpnEia4mMTdIih9HVMMVkqw";
    }
    axios
      .post(url, authData)
      .then((res) => {
  ...
  return (dispatch) => {
    ...
    if (!token) {
      ...
      if (expDate > new Date()) {
        ...
        // to logout
        dispatch(
          checkTimeout((expDate.getTime() - new Date().getTime()) / 1000)
        );
      } else {
        dispatch(logoutAction());
      }
